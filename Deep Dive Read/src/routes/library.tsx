  ...
  return (
    <PageShell>
      ...
      {listsQ.isLoading ? (
        ...
        <EmptyState
          title="No lists yet"
          description="When the extension pushes lists to Zoom Out, they'll appear here."
        />
      ) : (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {filtered.map((l) => (
            <ListCard key={l.id} list={l} movies={moviesByList.get(l.id) ?? []} />
          ))}
        </div>
      )}
      {(listsQ.data?.length ?? 0) > 0 && (
        <div className="mt-4 flex justify-end">
          <Pill>{filtered.length} list{filtered.length === 1 ? "" : "s"}</Pill>
        </div>
      )}
    </PageShell>
  );
